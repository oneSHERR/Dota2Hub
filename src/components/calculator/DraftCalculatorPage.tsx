      ...
      <div className="max-w-6xl mx-auto px-4 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.5fr] gap-6">
          ...
          <div className="space-y-4">
            {/* Enemy team */}
            <div className="rounded-2xl bg-dota-card/60 border border-red-500/15 p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-2">
                  {enemies.length > 0 && (
                    <>
                      <button onClick={shareDraft} className="text-xs font-body text-slate-500 hover:text-blue-400 transition-all duration-300" title="Поделиться">
                        <Share2 className="w-4 h-4" />
                      </button>
                      <button onClick={reset} className="text-xs font-body text-slate-500 hover:text-red-400 transition-all duration-300">
              ...
              <div className="space-y-1.5">
                {[0,1,2,3,4].map(i => {
                  ...
                  return hero ? (
                    <div key={i} className="flex items-center gap-2 p-2 rounded-xl bg-red-500/5 border border-red-500/10">
                    ...
                    <button key={i} onClick={() => setSelectingFor('enemy')}
                      className="w-full flex items-center gap-2 p-2.5 rounded-xl border border-dashed border-red-500/15 hover:border-red-500/30 hover:bg-red-500/5 transition-all duration-300 text-left">
            ...
            {/* Ally team */}
            <div className="rounded-2xl bg-dota-card/60 border border-emerald-500/15 p-5">
              ...
              <div className="space-y-1.5">
                {allies.map((hero, i) => (
                  <div key={i} className="flex items-center gap-2 p-2 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                    <img src={hero.icon} alt="" className="w-8 h-8 rounded" />
                    <AttrIcon attr={hero.primary_attr} size={14} />
                    <span className="text-sm font-body font-bold text-white flex-1">{hero.localized_name}</span>
                ...
                {allies.length < 5 && (
                  <button onClick={() => setSelectingFor('ally')}
                    className="w-full flex items-center gap-2 p-2.5 rounded-xl border border-dashed border-emerald-500/15 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all duration-300 text-left">
            ...
            <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                <button onClick={() => setPosFilter(null)} className={`px-3 py-2 rounded-xl text-sm font-body font-bold transition-all duration-300 ${posFilter === null ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}>Все</button>
                {[1,2,3,4,5].map(pos => (
                  <button key={pos} onClick={() => setPosFilter(pos)}
                    className={`px-3 py-2 rounded-xl text-sm font-body font-bold transition-all duration-300 ${posFilter === pos ? 'bg-dota-gold/15 text-dota-gold' : 'text-slate-500 hover:text-white'}`}>
                    {POS_ROLES[pos].label}
                  </button>
                ))}
              </div>
                ...
                <div className="space-y-1.5">
                  {history.map((h, i) => (
                    <button key={i} onClick={() => loadDraft(h)}
                      className="w-full flex items-center gap-2 p-2 rounded-xl bg-white/[0.03] hover:bg-white/6 transition-all duration-300 text-left">
          ...
          <div>
            {enemies.length === 0 ? (
              <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-12 text-center">
                ...
                <div className="flex gap-1 mb-6 p-1 rounded-2xl bg-dota-card/60 border border-dota-border/20 w-fit">
                  {[
                    ...
                    <button key={key} onClick={() => setActiveTab(key)}
                      className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-body font-bold transition-all duration-300 ${activeTab === key ? 'bg-dota-gold/15 text-dota-gold' : 'text-slate-500 hover:text-white'}`}>
                ...
                {activeTab === 'picks' && (
                  <div className="space-y-3">
                    ...
                    {recommendations.slice(0, 15).map((rec, i) => {
                      ...
                      return (
                        <div key={rec.hero.id}
                          className={`rounded-2xl bg-dota-card/60 border p-4 transition-all duration-300 hover:border-white/15 ${i < 3 ? 'border-dota-gold/20' : 'border-dota-border/20'}`}>
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 font-mono text-sm font-bold ${
                              i === 0 ? 'bg-amber-500/20 text-amber-400' : i === 1 ? 'bg-slate-400/20 text-slate-300' : i === 2 ? 'bg-orange-700/20 text-orange-400' : 'bg-white/[0.04] text-slate-500'
                            }`}>{i + 1}</div>

                            <img src={rec.hero.img} alt="" className="w-14 h-9 rounded-xl object-cover" />
                            ...
                            <div className="flex items-center gap-3 flex-shrink-0">
                              ...
                              <div className="w-3 h-8 rounded-full overflow-hidden bg-white/[0.04]">
                    ...
                    <div className="mt-8">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {[1,2,3,4,5].map(pos => {
                          ...
                          return (
                            <div key={pos} className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-4">
                              ...
                              <div className="space-y-2">
                                {posRecs.map((rec, i) => {
                                  const wr = rec.winRate ? (rec.winRate * 100).toFixed(1) : null;
                                  return (
                                    <div key={rec.hero.id} className="flex items-center gap-2.5 p-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.04] transition-all duration-300">
                                      <span className={`w-5 h-5 rounded flex items-center justify-center text-[10px] font-mono font-bold ${
                                        i === 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-white/[0.04] text-slate-500'
                                      }`}>{i + 1}</span>
                                      <img src={rec.hero.icon} alt="" className="w-8 h-8 rounded-xl" />
                                      <div className="flex-1 min-w-0">
                ...
                {activeTab === 'bans' && (
                  <div className="space-y-3">
                    ...
                    {banRecs.length > 0 ? banRecs.map((ban, i) => (
                      <div key={ban.hero.id} className="flex items-center gap-3 p-4 rounded-2xl bg-dota-card/60 border border-red-500/10 hover:border-red-500/20 transition-all duration-300">
                        <div className="w-8 h-8 rounded-xl bg-red-500/15 flex items-center justify-center text-sm font-mono font-bold text-red-400">{i + 1}</div>
                        <img src={ban.hero.icon} alt="" className="w-10 h-10 rounded-xl" />
                ...
                {activeTab === 'lanes' && (
                  <div className="space-y-3">
                    ...
                    {allies.length === 0 ? (
                      ...
                      <div key={lane.lane} className={`rounded-2xl bg-dota-card/60 border p-4 ${
                        lane.winner === 'ally' ? 'border-emerald-500/20' : lane.winner === 'enemy' ? 'border-red-500/20' : 'border-dota-border/20'
                      }`}>
                        <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <div className="flex gap-1.5">{lane.allyHeroes.map(h => (
                              <div key={h.id} className="flex items-center gap-1.5 px-2 py-1 rounded-xl bg-emerald-500/5">
                          ...
                          <div>
                            ...
                            <div className="flex gap-1.5">{lane.enemyHeroes.map(h => (
                              <div key={h.id} className="flex items-center gap-1.5 px-2 py-1 rounded-xl bg-red-500/5">
                                <img src={h.icon} alt="" className="w-6 h-6 rounded" />
                                <span className="text-xs font-body text-white">{h.localized_name}</span>
                              </div>
                ...
                {activeTab === 'dangers' && (
                  <div className="space-y-3">
                    ...
                    {enemies.length >= 2 && allies.length >= 1 && (
                      <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-5 mt-4">
                            ...
                            <div key={ally.id} className="flex items-center gap-2">
                              ...
                              <div className="flex-1 flex gap-1">
                                {enemies.map(enemy => {
                                  ...
                                  return (
                                    <div key={enemy.id} className="flex-1 flex items-center gap-1 px-2 py-1 rounded-xl" style={{ backgroundColor: heatColor(adv, 5) + '20' }}>
        ...
        {selectingFor && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={() => setSelectingFor(null)}>
            <div className="w-full max-w-4xl max-h-[85vh] bg-dota-card/60 border border-dota-border/30 rounded-t-2xl sm:rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="p-5 border-b border-dota-border/20">
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <input ref={searchInputRef} value={search} onChange={e => setSearch(e.target.value)} onKeyDown={handleSearchKeyDown}
                      ...
                      className="w-full pl-10 pr-3 py-3 rounded-2xl bg-white/[0.04] border border-dota-border/30 text-white font-body text-base placeholder:text-slate-600 focus:border-white/20 focus:outline-none" />
                  </div>
                  <div className="flex gap-1">
                    {[
                      ...
                      <button key={key} onClick={() => setAttrFilter(key)}
                        className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-300 ${attrFilter === key ? 'bg-white/15 ring-2 ring-white/20' : 'hover:bg-white/[0.04]'}`}
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                  {filteredHeroes.map(hero => (
                    <button key={hero.id} onClick={() => addHero(hero)}
                      className="group relative rounded-2xl overflow-hidden hover:scale-105 hover:z-10 hover:shadow-xl hover:shadow-black/40 transition-all duration-300 cursor-pointer border border-transparent hover:border-white/20"
