...
export function DraftCalculatorPage() {
  ...
  const addHero = useCallback((hero: Hero) => {
    ...
    else if (selectingFor === 'ally' && allies.length < 4) setAllies(prev => [...prev, hero]);
      ...
      <div className="max-w-6xl mx-auto px-4 pb-16">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-[#111827] border border-emerald-500/15 p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-body text-slate-500">{allies.length}/4 · необязательно</span>
              </div>
              <div className="space-y-1.5">
                {allies.map((hero, i) => (
        ...
        {selectingFor && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center" onClick={() => setSelectingFor(null)}>
            <div className="w-full max-w-2xl max-h-[80vh] bg-[#111827] border border-white/10 rounded-t-2xl sm:rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input ref={searchInputRef} value={search} onChange={e => setSearch(e.target.value)} onKeyDown={handleSearchKeyDown}
                      ...
                      className="w-full pl-9 pr-3 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white font-body text-base placeholder:text-slate-600 focus:border-white/20 focus:outline-none" />
                  </div>
                  <div className="flex gap-0.5">
                    {[
                      ...
                      <button key={key} onClick={() => setAttrFilter(key)}
                        className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all ${attrFilter === key ? 'bg-white/15 ring-1 ring-white/20' : 'hover:bg-white/5'}`}
                        title={key === 'all' ? 'Все' : key === 'uni' ? 'Универсал' : key === 'str' ? 'Сила' : key === 'agi' ? 'Ловкость' : 'Интеллект'}>
                        {label ? <span className="text-xs font-bold text-white">{label}</span> : <AttrIcon attr={attr!} size={18} />}
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  {filteredHeroes.map(hero => (
                    <button key={hero.id} onClick={() => addHero(hero)}
                      ...
                      <div className="absolute top-0.5 left-0.5">
                        <AttrIcon attr={hero.primary_attr} size={10} />
                      </div>
                      <span className="absolute bottom-0 left-0 right-0 text-[8px] font-body font-bold text-white text-center p-0.5 truncate drop-shadow-lg">
                        {hero.localized_name}
                      </span>
                    </button>
                  ))}
                </div>
                {filteredHeroes.length === 0 && <p className="text-center text-sm font-body text-slate-500 py-8">Герой не найден</p>}
              </div>
            </div>
          </div>
        )}
