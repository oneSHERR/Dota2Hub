...
export function DraftResult({ data, onNewGame, onRematch }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      {/* Winner banner */}
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[#111827] to-[#0f172a] border border-dota-border/20 p-8 text-center">
        ...
        <div className="relative z-10">
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="h-4 rounded-full bg-white/[0.04] overflow-hidden flex">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 rounded-l-full transition-all duration-300 duration-1000" style={{ width: `${analysis.player1Score}%` }} />
              <div className="h-full bg-gradient-to-r from-red-400 to-red-500 rounded-r-full transition-all duration-300 duration-1000" style={{ width: `${analysis.player2Score}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-2xl bg-dota-card/60 border border-dota-border/20 w-fit">
        {[
          ...
          <button key={key} onClick={() => setActiveTab(key)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-body font-bold transition-all duration-300 ${
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {analysis.keyMatchups && analysis.keyMatchups.length > 0 && (
            <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-6">
              ...
              <div className="space-y-2">
                {analysis.keyMatchups.map((m: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 p-3 rounded-2xl bg-white/2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-5 text-center">
              ...
              <div className="space-y-3">
                {analysis.laneBreakdown.map((lane: any, i: number) => (
                  <div key={i} className={`rounded-2xl p-4 border ${
                    ...
                    'bg-white/2 border-dota-border/20'
                  }`}>
      ...
      {activeTab === 'items' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-2xl bg-dota-card/60 border border-emerald-500/15 p-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4">
        <button onClick={onNewGame} className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-gradient-to-r from-dota-accent to-red-600 text-white font-body font-bold text-lg shadow-xl shadow-dota-accent/20 hover:scale-105 transition-all duration-300">
          <RotateCcw className="w-5 h-5" /> Новая игра
        </button>
        <button onClick={shareDraft} className="inline-flex items-center gap-2 px-6 py-4 rounded-2xl bg-white/[0.04] border border-dota-border/30 text-white font-body font-bold hover:bg-white/[0.08] transition-all duration-300">
...
function TeamSummary({ player, color, advantages, weaknesses }: {
  ...
  return (
    <div className={`rounded-2xl bg-dota-card/60 border ${borderClass} p-5`}>
      <div className="flex items-center gap-2 mb-4">
      ...
      <div className="space-y-1 mb-4">
        {slots.map((slot: any, i: number) => (
          <div key={i} className="flex items-center gap-2 p-2 rounded-xl bg-white/2">
