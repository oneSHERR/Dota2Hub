  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-dota-card border border-dota-border p-8 text-center">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-display text-3xl font-black text-white mb-2">
            {winner === 'draw' ? 'Ничья!' : `${winnerName} побеждает!`}
          </h1>
          ...
          <div className="mt-6 max-w-md mx-auto">
            ...
            <div className="h-4 rounded-full bg-dota-bg overflow-hidden flex">
              <div
                className="h-full bg-gradient-to-r from-dota-radiant to-green-600 transition-all duration-1000"
                style={{ width: `${analysis.player1Score}%` }}
              />
              <div
                className="h-full bg-gradient-to-r from-red-700 to-dota-dire transition-all duration-1000"
                style={{ width: `${analysis.player2Score}%` }}
              />
            </div>
            <div className="text-center mt-2">
              <span className="text-xs font-body text-slate-500">
                Уверенность: {analysis.confidence > 15 ? 'Высокая' : analysis.confidence > 5 ? 'Средняя' : 'Низкая'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <TeamSummary player={p1} label="Команда 1" color="radiant" advantages={analysis.player1Advantages} />
        <TeamSummary player={p2} label="Команда 2" color="dire" advantages={analysis.player2Advantages} />
      </div>

      {/* Lane breakdown */}
      {analysis.laneBreakdown.length > 0 && (
        <div className="rounded-2xl bg-dota-card border border-dota-border p-6">
          ...
          <div className="space-y-3">
            {analysis.laneBreakdown.map((lane, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-dota-bg/50">
                ...
                <div className="flex-1 flex items-center gap-2">
                  <span className="text-xs font-body text-dota-radiant">{lane.team1Heroes.join(', ')}</span>
                  <span className="text-xs text-slate-600">vs</span>
                  <span className="text-xs font-body text-dota-dire">{lane.team2Heroes.join(', ')}</span>
                </div>
                <span className={`text-xs font-body font-bold px-2 py-1 rounded ${
                  lane.advantage === 'team1' ? 'bg-green-500/15 text-green-400'
          ...
          <div className="space-y-2">
            {analysis.keyMatchups.map((m, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-dota-bg/50">
                <span className={`font-body text-sm font-bold ${m.advantage > 0 ? 'text-green-400' : 'text-red-400'}`}>
                  {m.hero1}
                </span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl bg-dota-card border border-dota-border p-4 text-center">
          ...
          <div className="font-display text-2xl font-bold text-white">{analysis.synergyScore.team1}</div>
        ...
        <div className="rounded-xl bg-dota-card border border-dota-border p-4 text-center">
          ...
          <div className="font-display text-2xl font-bold text-white">{analysis.synergyScore.team2}</div>
      ...
      <div className="text-center pt-4">
        <button
          onClick={onNewGame}
          className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-gradient-to-r from-dota-accent to-red-700 text-white font-body font-bold text-lg shadow-xl shadow-dota-accent/20 hover:shadow-dota-accent/40 hover:scale-105 transition-all"
        >
...
function TeamSummary({
  ...
  const borderClass = color === 'radiant' ? 'border-dota-radiant/30' : 'border-dota-dire/30';
  const dotClass = color === 'radiant' ? 'bg-dota-radiant' : 'bg-dota-dire';

  return (
    <div className={`rounded-2xl bg-dota-card border ${borderClass} p-5`}>
      <div className="flex items-center gap-2 mb-4">
      ...
      <div className="space-y-1.5 mb-4">
        {player?.slots?.map((slot: any, i: number) => (
