  ...
  if (phase === 'setup') {
    return (
      <div className="max-w-2xl mx-auto py-12 px-4">
        <button onClick={onBack} className="flex items-center gap-2 text-base font-body text-slate-400 hover:text-white mb-8 transition-colors">
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <div className={`px-4 py-2 rounded-lg font-body text-sm font-bold flex items-center gap-2 ${actionType === 'ban' ? 'bg-red-500/15 text-red-400' : 'bg-emerald-500/15 text-emerald-400'}`}>
        ...
        <button onClick={onBack} className="px-3 py-1.5 rounded-lg bg-slate-800/80 text-slate-400 text-xs font-body hover:bg-slate-700 transition-colors">Выйти</button>
      </div>

      {/* Progress */}
      <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
        <div className="h-full rounded-full bg-gradient-to-r from-red-500 via-dota-gold to-emerald-500 transition-all duration-500" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="flex items-center gap-0.5 overflow-x-auto pb-1">
        {sequence.map((step, i) => {
          ...
          return (
            <div key={i} className={`flex-shrink-0 w-7 h-7 rounded-md flex items-center justify-center text-[9px] font-mono font-bold border transition-all ${
      ...
      {allBans.length > 0 && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-red-500/5 border border-red-500/10">
          ...
          <div className="flex flex-wrap gap-2">
            {allBans.map((h, i) => (
              <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-red-500/10">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск героя..."
              className="w-full pl-10 pr-8 py-2.5 rounded-lg bg-white/5 border border-white/8 text-white font-body text-base placeholder:text-slate-600 focus:border-white/15 focus:outline-none" />
          ...
          <div className="flex gap-1">
            {[
              ...
              <button key={key} onClick={() => setAttrFilter(key)}
                className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${attrFilter === key ? 'bg-white/15 ring-1 ring-white/20' : 'hover:bg-white/5'}`}>
          ...
          {filteredHeroes.map(hero => {
            ...
            return (
              <button key={hero.id} onClick={() => handleHeroClick(hero)} disabled={!canClick}
                className={`group relative rounded-lg overflow-hidden transition-all duration-150 ${canClick ? 'hover:scale-110 hover:z-10 hover:shadow-xl cursor-pointer' : 'opacity-40 cursor-not-allowed'}`}
