...
import { Search, X, Ban, Clock, Shield, Swords, Timer } from 'lucide-react';
...
export function DraftGame({ roomInfo, onBattleStart, onLeave }: Props) {
  ...
  }, [roomData?.currentTurn, roomData?.phase, roomData?.turnNumber]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          {/* Timer */}
          {isMyTurn && (
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-mono text-sm font-bold ${timer <= 10 ? 'bg-red-500/15 text-red-400 animate-pulse' : timer <= 20 ? 'bg-amber-500/10 text-amber-400' : 'bg-white/5 text-slate-300'}`}>
