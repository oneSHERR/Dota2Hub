...
export function DraftPage() {
  ...
  const handleGameEnd = (data: any) => { setResultData(data); setView('result'); };
  const handleNewGame = () => { setRoomInfo(null); setResultData(null); setView('lobby'); };

  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      {view === 'lobby' && <DraftLobby onStartGame={handleStartGame} />}
      {view === 'game' && roomInfo && <DraftGame roomInfo={roomInfo} onGameEnd={handleGameEnd} onLeave={handleNewGame} />}
