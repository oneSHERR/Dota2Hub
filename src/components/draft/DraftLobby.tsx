...
export function DraftLobby({ onStartGame }: Props) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToRoom(waitingRoom, (data) => {
      ...
      if (data?.player1?.isReady && data?.player2?.isReady) {
        ...
        if (isP1) {
          updateRoom(waitingRoom, { phase: 'ban', currentTurn: 'player1', turnNumber: 1 });
        }
        setTimeout(() => {
  ...
  const handleCreate = async () => {
    ...
    try {
      await createRoom(roomId, {
        roomId, phase: 'lobby',
        player1: {
          uid: user.uid, name: user.displayName || 'Player 1', isReady: false,
          slots: [{ position: 1, hero: null }, { position: 2, hero: null }, { position: 3, hero: null }, { position: 4, hero: null }, { position: 5, hero: null }],
          bans: [],
        },
      ...
      setIsCreator(true);
    } catch (err) { setError('Не удалось создать комнату'); }
    setLoading(false);
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      const unsub = subscribeToRoom(roomId, async (data) => {
        ...
        await updateRoom(roomId, {
          player2: {
            uid: user.uid, name: user.displayName || 'Player 2', isReady: false,
            slots: [{ position: 1, hero: null }, { position: 2, hero: null }, { position: 3, hero: null }, { position: 4, hero: null }, { position: 5, hero: null }],
            bans: [],
          },
        });
    ...
    } catch (err) { setError('Ошибка подключения'); setLoading(false); }
  };
  ...
  const copyCode = () => {
    if (waitingRoom) { navigator.clipboard.writeText(waitingRoom); setCopied(true); setTimeout(() => setCopied(false), 2000); }
  };
    ...
    return (
      <div className="max-w-lg mx-auto py-12">
        <div className="rounded-2xl bg-dota-card border border-dota-border p-8 text-center">
          ...
          {isCreator && (
            <div className="flex items-center justify-center gap-2 mb-6">
              <div className="px-6 py-3 rounded-xl bg-dota-bg border border-dota-border">
                <span className="font-mono text-3xl font-bold text-dota-gold tracking-[0.3em]">{waitingRoom}</span>
              </div>
              <button onClick={copyCode} className="p-3 rounded-xl bg-dota-bg border border-dota-border hover:border-dota-gold/30 transition-colors">
          ...
          {p2 && !meReady && (
            <button onClick={handleReady} className="w-full py-3 rounded-xl bg-gradient-to-r from-dota-radiant to-green-700 text-white font-body font-bold hover:scale-[1.02] transition-transform">Готов!</button>
          )}
          {meReady && (
            <div className="py-3 rounded-xl bg-green-500/10 border border-green-500/20">
              <span className="text-sm font-body text-green-400">{p1?.isReady && p2?.isReady ? 'Оба готовы! Запускаем...' : 'Ждём готовности соперника...'}</span>
            </div>
          )}
          {!p2 && isCreator && (
  ...
  return (
    <div className="max-w-2xl mx-auto py-12">
      <div className="text-center mb-10">
        ...
        <p className="text-sm font-body text-slate-400">Создай комнату или подключись к другу</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button onClick={handleCreate} disabled={loading} className="group rounded-2xl bg-dota-card border border-dota-border p-8 text-center hover:border-dota-accent/30 transition-all">
        ...
        <div className="rounded-2xl bg-dota-card border border-dota-border p-8 text-center">
          ...
          <div className="flex gap-2">
            <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} placeholder="КОД" maxLength={6} className="flex-1 px-4 py-3 rounded-xl bg-dota-bg border border-dota-border text-white font-mono text-center text-lg tracking-widest placeholder:text-slate-600 focus:border-blue-500/50 focus:outline-none uppercase" />
      ...
      {error && <div className="mt-4 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-center"><span className="text-sm font-body text-red-400">{error}</span></div>}
      <div className="mt-12 rounded-2xl bg-dota-card/50 border border-dota-border/50 p-6">
        ...
        <div className="space-y-3 text-sm font-body text-slate-400">
          <div className="flex items-start gap-3"><span className="w-6 h-6 rounded-full bg-dota-accent/20 text-dota-accent flex items-center justify-center text-xs font-bold flex-shrink-0">1</span><span>Создай комнату и отправь код другу</span></div>
          <div className="flex items-start gap-3"><span className="w-6 h-6 rounded-full bg-dota-accent/20 text-dota-accent flex items-center justify-center text-xs font-bold flex-shrink-0">2</span><span>Каждый банит по 2 героя</span></div>
          <div className="flex items-start gap-3"><span className="w-6 h-6 rounded-full bg-dota-accent/20 text-dota-accent flex items-center justify-center text-xs font-bold flex-shrink-0">3</span><span>Выбирайте по 5 героев — по одному на позицию (1-5)</span></div>
          <div className="flex items-start gap-3"><span className="w-6 h-6 rounded-full bg-dota-accent/20 text-dota-accent flex items-center justify-center text-xs font-bold flex-shrink-0">4</span><span>AI анализирует связки и определяет победителя</span></div>
        </div>
      </div>
    </div>
  );
...
function PlayerStatus({ name, isReady, connected, isYou }: { name: string; isReady: boolean; connected: boolean; isYou: boolean }) {
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl ${connected ? 'bg-dota-bg/50' : 'bg-dota-bg/20'}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${connected ? isReady ? 'bg-green-500/20' : 'bg-dota-gold/20' : 'bg-slate-700/50'}`}>
        {connected ? isReady ? <UserCheck className="w-4 h-4 text-green-400" /> : <Users className="w-4 h-4 text-dota-gold" /> : <UserX className="w-4 h-4 text-slate-600" />}
      </div>
      ...
      {connected && <span className={`text-xs font-body px-2 py-0.5 rounded ${isReady ? 'bg-green-500/15 text-green-400' : 'bg-slate-700 text-slate-400'}`}>{isReady ? 'Готов' : 'Не готов'}</span>}
    </div>
  );
}
