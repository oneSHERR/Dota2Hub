...
// ========== CAPTAIN'S MODE SEQUENCE GENERATOR ==========
function generateDraftSequence() {
  // Рандомизируем кто начинает
  const first = Math.random() > 0.5 ? 'player1' : 'player2';
  const second = first === 'player1' ? 'player2' : 'player1';

  // Captain's Mode с 2 банами на игрока (4 бана + 10 пиков = 14 шагов)
  // Порядок: бан-бан-бан-бан, потом пики с двойными ходами
  return [
    // Бан фаза
    ...
    { type: 'ban', player: second },
    { type: 'ban', player: first },
    // Пик фаза — с двойными пиками как в реальной Dota
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToRoom(waitingRoom, (data) => {
      ...
      if (data?.player1?.isReady && data?.player2?.isReady) {
        ...
        if (isP1) {
          // Генерируем последовательность драфта и записываем в Firebase
          const seq = generateDraftSequence();
          updateRoom(waitingRoom, {
            phase: 'draft',
            currentTurn: seq[0].player,
            currentStep: 0,
            draftSequence: seq,
            turnNumber: 1,
          });
  ...
  const handleCreate = async () => {
    ...
    try {
      await createRoom(roomId, {
        ...
        player2: null, currentTurn: null, currentStep: 0, draftSequence: [], turnNumber: 0, banCount: 2, timer: 30, winner: null, analysis: null, createdAt: Date.now(),
      });
        ...
        <div className="rounded-2xl bg-dota-card/60 border border-white/8 p-8 text-center">
          ...
          {isCreator && (
            <div className="flex items-center justify-center gap-2 mb-6">
              <div className="px-6 py-3 rounded-2xl bg-black/30 border border-dota-border/30"><span className="font-mono text-3xl font-bold text-dota-gold tracking-[0.3em]">{waitingRoom}</span></div>
              <button onClick={copyCode} className="p-3 rounded-2xl bg-black/30 border border-dota-border/30 hover:border-dota-gold/30 transition-all duration-300">{copied ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5 text-slate-400" />}</button>
            </div>
          )}
          ...
          {p2 && !meReady && <button onClick={handleReady} className="w-full py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-green-600 text-white font-body font-bold hover:scale-[1.02] transition-transform shadow-lg shadow-emerald-500/20">Готов!</button>}
          {meReady && <div className="py-3 rounded-2xl bg-emerald-500/8 border border-emerald-500/15"><span className="text-sm font-body text-emerald-400">{p1?.isReady && p2?.isReady ? 'Оба готовы! Генерация Captain\'s Mode...' : 'Ждём соперника...'}</span></div>}
  ...
  return (
    <div className="max-w-2xl mx-auto py-12">
      <div className="text-center mb-10">
        ...
        <p className="text-base font-body text-slate-400">Captain's Mode · 2 бана · Рандомный порядок · AI-анализ</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button onClick={handleCreate} disabled={loading} className="group rounded-2xl bg-dota-card/60 border border-white/8 p-8 text-center hover:border-emerald-500/20 transition-all duration-300">
          ...
          <p className="text-sm font-body text-slate-500">Получи код и отправь другу</p>
        </button>
        <div className="rounded-2xl bg-dota-card/60 border border-white/8 p-8 text-center">
          ...
          <div className="flex gap-2">
            <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} placeholder="КОД" maxLength={6} className="flex-1 px-4 py-3 rounded-2xl bg-black/30 border border-dota-border/30 text-white font-mono text-center text-lg tracking-widest placeholder:text-slate-700 focus:border-blue-500/40 focus:outline-none uppercase" />
            <button onClick={handleJoin} disabled={loading || !joinCode.trim()} className="px-5 py-3 rounded-2xl bg-blue-600 text-white font-body font-bold text-sm hover:bg-blue-500 disabled:opacity-40 transition-all duration-300">Войти</button>
          </div>
        </div>
      </div>
      {error && <div className="mt-4 p-3 rounded-xl bg-red-500/8 border border-red-500/15 text-center"><span className="text-sm font-body text-red-400">{error}</span></div>}
      <div className="mt-10 rounded-2xl bg-white/2 border border-dota-border/20 p-6">
        <h3 className="font-display text-lg font-bold text-white mb-4">Captain's Mode</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          {['Создай комнату', 'Рандом: кто первый', 'Бан-бан-пик-пик', 'AI определит победу'].map((t, i) => (
...
function PlayerStatus({ name, isReady, connected, isYou }: { name: string; isReady: boolean; connected: boolean; isYou: boolean }) {
  return (
    <div className={`flex items-center gap-3 p-3 rounded-2xl ${connected ? 'bg-white/[0.03]' : 'bg-white/1'}`}>
      ...
      {connected && <span className={`text-[10px] font-body px-2 py-0.5 rounded ${isReady ? 'bg-emerald-500/12 text-emerald-400' : 'bg-white/[0.04] text-slate-400'}`}>{isReady ? 'Готов' : 'Не готов'}</span>}
    </div>
  );
}
