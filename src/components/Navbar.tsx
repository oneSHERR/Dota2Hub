...
import { Swords, BookOpen, HelpCircle, LogIn, Menu, X, Home, ScrollText, Crown, Calculator, Target, Eye } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group flex-shrink-0">
          ...
          <div className="hidden sm:block">
            <h1 className="font-display text-xl font-bold gradient-text tracking-wider">DOTA 2 HUB</h1>
            <p className="text-[10px] text-slate-500 font-body tracking-widest uppercase -mt-0.5">v3 · Полный арсенал</p>
          </div>
        </Link>
            ...
            return (
              <Link key={path} to={path}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-body font-semibold transition-all ${
                  active ? 'bg-dota-accent/15 text-dota-accent' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <Link to="/profile" className="flex items-center gap-2 px-3 py-2 rounded-lg bg-dota-card border border-dota-border hover:border-dota-gold/30 transition-colors">
              ...
              <span className="text-sm font-body text-slate-300 hidden sm:block">{user.displayName}</span>
            </Link>
          ) : (
            <Link to="/auth" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-dota-accent/15 text-dota-accent hover:bg-dota-accent/25 transition-colors text-sm font-body font-medium">
