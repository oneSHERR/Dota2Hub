...
import { Swords, BookOpen, HelpCircle, User, LogIn, Menu, X, Home, ScrollText, Trophy, Crown, Target, Calculator } from 'lucide-react';

const NAV_ITEMS = [
  { path: '/', label: 'Главная', icon: Home },
  { path: '/draft', label: 'Draft Arena', icon: Swords },
  ...
  { path: '/tierlist', label: 'Тир-лист', icon: Trophy },
  { path: '/leaderboard', label: 'Рейтинг', icon: Crown },
  { path: '/quiz', label: 'Квизы', icon: HelpCircle },
];
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <div className="hidden sm:block">
            <h1 className="font-display text-lg font-bold gradient-text tracking-wider">
              DOTA 2 HUB
            </h1>
            <p className="text-[10px] text-slate-500 font-body tracking-widest uppercase -mt-1">
              Draft · Wiki · Tier List
            </p>
          </div>
        </Link>
        ...
        <nav className="lg:hidden border-t border-dota-border/50 bg-dota-bg/95 backdrop-blur-xl animate-fade-in">
          {NAV_ITEMS.map(({ path, label, icon: Icon }) => (
            <Link
              ...
              className={`flex items-center gap-3 px-6 py-3 text-sm font-body ${
