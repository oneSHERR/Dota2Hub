...
import { Swords, BookOpen, HelpCircle, User, LogIn, Menu, X, Home, ScrollText } from 'lucide-react';
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-dota-border/50 backdrop-blur-xl bg-dota-bg/80">
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <nav className="md:hidden border-t border-dota-border/50 bg-dota-bg/95 backdrop-blur-xl animate-fade-in">
          {NAV_ITEMS.map(({ path, label, icon: Icon }) => (
            <Link
              key={path}
              to={path}
