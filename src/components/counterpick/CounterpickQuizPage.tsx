    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-2xl mx-auto px-4 pb-16 pt-6">
          {/* How to play */}
          <div className="rounded-2xl bg-[#111827] border border-white/8 p-6 mb-6">
            ...
            {(['easy', 'medium', 'hard'] as Difficulty[]).map(diff => {
              ...
              return (
                <button
                  ...
                  className="w-full group rounded-2xl bg-[#111827] border border-white/8 p-5 text-left hover:border-dota-gold/20 transition-all"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <ChevronRight className="w-5 h-5 text-slate-600 group-hover:text-dota-gold group-hover:translate-x-1 transition-all" />
                  </div>
                </button>
              );
            })}
          ...
          <div className="rounded-2xl bg-[#111827] border border-white/8 p-8 text-center">
            ...
            <div className="space-y-2">
              <button onClick={() => startGame(difficulty)}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-dota-accent to-red-600 text-white font-body font-bold hover:scale-[1.02] transition-transform">
                <RotateCcw className="w-4 h-4 inline mr-2" /> Играть снова
              </button>
              <button onClick={() => setPhase('menu')}
                className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-slate-300 font-body font-bold hover:bg-white/10 transition-colors">
                Сменить сложность
              </button>
            </div>
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-dota-gold/10">
            <Star className="w-3.5 h-3.5 text-dota-gold" />
            <span className="font-mono text-sm font-bold text-dota-gold">{score}</span>
          </div>
          {streak >= 2 && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-500/10 animate-pulse">
          ...
          {config.timePerRound > 0 && phase === 'playing' && (
            <div className={`flex items-center gap-1 px-3 py-1 rounded-lg font-mono text-sm font-bold ${
              timer <= 5 ? 'bg-red-500/15 text-red-400 animate-pulse' : 'bg-white/5 text-slate-300'
            }`}>
      ...
      {/* Enemy hero */}
      <div className="rounded-2xl bg-[#111827] border border-red-500/20 p-6 mb-6 text-center">
        <span className="text-xs font-body text-red-400 uppercase tracking-wider mb-3 block">Враг выбрал</span>
        <div className="w-32 h-20 rounded-xl overflow-hidden border-2 border-red-500/30 mx-auto mb-3 shadow-xl shadow-red-500/10">
        ...
        {round.options.map((option, i) => {
          ...
          return (
            <button
              ...
              className={`relative rounded-xl ${bgColor} border ${borderColor} p-4 transition-all ${
      ...
      {phase === 'answered' && (
        <div className="space-y-3" style={{ animation: 'fadeIn 0.3s ease-out' }}>
          ...
          <button onClick={nextRound}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-dota-gold/80 to-amber-600 text-white font-body font-bold hover:scale-[1.02] transition-transform">
