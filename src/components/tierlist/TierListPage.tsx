      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        ...
        <div className="sticky top-0 z-20 bg-dota-bg/95 backdrop-blur-md border-b border-dota-border/20 -mx-4 px-4 py-3 mb-6">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <div className="flex gap-1">
              {[
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-body font-bold uppercase tracking-wider transition-all duration-300"
                  style={{
            ...
            {/* Rank filter */}
            <div className="flex gap-1 border border-dota-border/20 rounded-xl p-0.5">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-body font-bold transition-all duration-300 ${rankFilter === key ? 'bg-dota-gold/15 text-dota-gold' : 'text-slate-500 hover:text-white'}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="flex gap-1 border border-dota-border/20 rounded-xl p-0.5">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-sm font-body font-bold transition-all duration-300 ${sortBy === key ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}
                >
            ...
            {/* Refresh */}
            <button onClick={loadData} className="p-2.5 rounded-xl bg-white/[0.04] border border-dota-border/30 text-slate-400 hover:text-white hover:bg-white/[0.08] transition-all duration-300" title="Обновить данные">
                  ...
                  return (
                    <div key={tier}>
                      ...
                      <div className="flex items-center gap-4 mb-4">
                        <div
                          className="w-14 h-14 rounded-2xl flex items-center justify-center font-display text-2xl font-black"
...
function TierHeroCard({ data, tierColor }: { data: TierHero; tierColor: string }) {
  ...
  return (
    <div
      className="group relative rounded-2xl bg-gradient-to-br from-[#111827] to-[#0d1117] border overflow-hidden hover:scale-[1.03] hover:shadow-2xl hover:shadow-black/40 transition-all duration-300"
    ...
    >
      ...
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        {/* Тир бейдж */}
        <div
          className="absolute top-2 right-2 w-7 h-7 rounded-xl flex items-center justify-center font-display text-sm font-black"
...
function TierListRow({ data, rank }: { data: TierHero; rank: number }) {
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-2xl bg-dota-card/60/60 border border-white/3 hover:bg-dota-card/60 hover:border-white/8 transition-all duration-300">
      {/* Rank */}
      <span className="w-8 text-right text-sm font-mono font-bold text-slate-600">#{rank}</span>

      {/* Hero */}
      <img src={data.hero.icon} alt="" className="w-10 h-10 rounded-xl" loading="lazy" />
      ...
      {/* Bar */}
      <div className="w-24 h-2 rounded-full bg-white/[0.04] overflow-hidden hidden md:block">
        <div
          className="h-full rounded-full transition-all duration-300 duration-500"
          style={{
