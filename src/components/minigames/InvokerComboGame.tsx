...
export function InvokerComboGame({ onBack, onSave, user, bestScore }: GameProps) {
  ...
  const r = useRef({ orbs: [] as string[], target: null as Spell | null, score: 0, combo: 0, maxCombo: 0, time: 30, over: false, started: false });
  useEffect(() => { r.current.orbs = orbs; }, [orbs]);
  useEffect(() => { r.current.target = targetSpell; }, [targetSpell]);
  useEffect(() => { r.current.over = gameOver; }, [gameOver]);
  useEffect(() => { r.current.started = gameStarted; }, [gameStarted]);

  const pickTarget = useCallback(() => {
    const sp = SPELLS[randInt(0, SPELLS.length - 1)];
    setTargetSpell(sp); r.current.target = sp;
  }, []);

  const startGame = useCallback(() => {
    r.current = { orbs: [], target: null, score: 0, combo: 0, maxCombo: 0, time: 30, over: false, started: true };
    setScore(0); setCombo(0); setMaxCombo(0); setTimeLeft(30);
    setOrbs([]); setInvokedSpell(null); setSpellsCast(0); setFlash(null);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      r.current.time--;
      setTimeLeft(r.current.time);
      if (r.current.time <= 0) {
        clearInterval(id);
        r.current.over = true;
        setGameOver(true);
        if (user) onSave('invoker-combo', r.current.score, r.current.maxCombo);
      }
    }, 1000);
    return () => clearInterval(id);
  }, [gameStarted, gameOver]);

  // Keyboard — single stable effect
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (!r.current.started || r.current.over) return;
      const k = e.key.toUpperCase();
      if (['Q','W','E','R','H'].includes(k)) e.preventDefault();

      if (k === 'Q' || k === 'W' || k === 'E') {
        const next = [...r.current.orbs, k];
      ...
      if (k === 'R') {
        const spell = findSpell(r.current.orbs);
        if (!spell || !r.current.target) return;
        setInvokedSpell(spell);

        if (spell.name === r.current.target.name) {
          setFlash('correct');
          r.current.combo++;
          if (r.current.combo > r.current.maxCombo) r.current.maxCombo = r.current.combo;
          r.current.score += 50 + r.current.combo * 10;
          r.current.time = Math.min(r.current.time + 2, 90);
          setScore(r.current.score); setCombo(r.current.combo);
          setMaxCombo(r.current.maxCombo); setTimeLeft(r.current.time);
          setSpellsCast(n => n + 1);
          setTimeout(() => { setFlash(null); setInvokedSpell(null); pickTarget(); }, 250);
        } else {
          setFlash('wrong');
          r.current.combo = 0; setCombo(0);
          setTimeout(() => setFlash(null), 350);
        }
        return;
      }

      if (k === 'H') setShowHints(v => !v);
    };
    ...
    return (
      <GameWrapper title="Инвокер" icon={Zap} color="from-amber-500 to-orange-600"
        ...
        <div className="text-center py-10">
          ...
          <h3 className="font-display text-3xl font-bold text-white mb-4">Invoker Trainer</h3>
          <div className="max-w-sm mx-auto text-left space-y-3 mb-8">
            ...
            <p className="flex items-center gap-3 text-sm font-body text-slate-300">
              <kbd className="px-2 py-1 rounded bg-dota-card border border-dota-gold/40 text-xs font-mono font-bold text-dota-gold">R</kbd>
              <span className="text-slate-500">— Invoke + каст</span>
            </p>
            <p className="flex items-center gap-3 text-sm font-body text-slate-300">
              <kbd className="px-2 py-1 rounded bg-dota-card border border-dota-border text-xs font-mono font-bold text-white">H</kbd>
              <span className="text-slate-500">— подсказки</span>
            </p>
          </div>
    ...
    return (
      <GameWrapper title="Инвокер" icon={Zap} color="from-amber-500 to-orange-600"
        ...
        <div className="text-center py-10">
          ...
          <p className="text-slate-400 font-body mb-1">Счёт: <span className="text-dota-gold font-bold text-xl">{score}</span></p>
          <p className="text-slate-500 font-body text-sm mb-1">Макс. комбо: {maxCombo} · Заклинаний: {spellsCast}</p>
          <button onClick={startGame} className="mt-6 px-8 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-white font-body font-bold hover:scale-105 transition-transform">Ещё раз</button>
        </div>
      </GameWrapper>
    );
  }
  ...
  return (
    <GameWrapper title="Инвокер" icon={Zap} color="from-amber-500 to-orange-600"
      ...
      <div className={`transition-all duration-100 rounded-2xl ${flash === 'correct' ? 'ring-2 ring-green-500/50' : flash === 'wrong' ? 'ring-2 ring-red-500/50' : ''}`}>
        ...
        {/* Timer */}
        <div className="relative h-2.5 rounded-full bg-dota-card overflow-hidden border border-dota-border/30 mb-6">
          <div className="h-full rounded-full transition-all duration-500" style={{
            width: `${Math.max(0, (timeLeft / 30) * 100)}%`,
            background: timeLeft > 10 ? 'linear-gradient(90deg, #f59e0b, #ef4444)' : '#ef4444',
          }} />
        </div>
        ...
        <div className="flex justify-center mb-3">
          <button onClick={() => setShowHints(v => !v)}
            ...
            {showHints ? 'Скрыть подсказки (H)' : 'Подсказки (H)'}
          </button>
        </div>

        {/* Hints grid */}
        {showHints && (
          <div className="rounded-2xl bg-dota-card/50 border border-dota-border/20 p-3 animate-fade-in">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5">
              {SPELLS.map(sp => {
                const isT = targetSpell?.name === sp.name;
                return (
                  <div key={sp.name} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg ${isT ? 'bg-dota-gold/10 border border-dota-gold/20' : 'bg-dota-bg/30'}`}>
                    <img src={abilIcon(sp.icon)} alt="" className="w-7 h-7 rounded-lg flex-shrink-0" />
                    <div>
                      <p className={`text-[10px] font-body font-bold truncate ${isT ? 'text-dota-gold' : 'text-slate-400'}`}>{sp.name}</p>
                      <div className="flex gap-0.5">{sp.combo.map((o,i) => <span key={i} className="text-[9px] font-bold" style={{ color: ORB_COLORS[o].border }}>{o}</span>)}</div>
                    </div>
                  </div>
                );
              })}
