...
export function InvokerComboGame({ onBack, onSave, user, bestScore }: GameProps) {
  ...
  // Keyboard — supports both EN and RU layouts
  useEffect(() => {
    // Map Russian keys to English equivalents
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      // Resolve key: try e.code first (layout-independent), then fallback to key mapping
      let k = '';
      const code = e.code;
      if (code === 'KeyQ') k = 'Q';
      else if (code === 'KeyW') k = 'W';
      else if (code === 'KeyE') k = 'E';
      else if (code === 'KeyR') k = 'R';
      else if (code === 'KeyH') k = 'H';
      else {
        // Fallback: map by character (for virtual keyboards etc)
        const raw = e.key;
        k = RU_MAP[raw] || raw.toUpperCase();
      }
      ...
      if (k === 'R') {
        ...
        if (spell.name === target.name) {
          ...
          if (ref.current.idx + 1 >= ref.current.queue.length) {
            ...
            ref.current.idx++;
            setCurrentIndex(ref.current.idx);
            setInvokedSpell(null);
          }
        } else {
