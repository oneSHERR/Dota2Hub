...
export function GuessHeroGame({ onBack, onSave, user, bestScore }: GameProps) {
  ...
  useEffect(() => {
    if (!question || question.questionMode !== 'silhouette' || !canvasRef.current) return;
    ...
    img.onload = () => {
      ...
      if (!selected) {
        ...
        for (let i = 0; i < d.length; i += 4) {
          ...
          if (brightness > 30) { d[i] = 20; d[i+1] = 15; d[i+2] = 40; d[i+3] = Math.min(255, brightness + 100); }
          else { d[i+3] = 0; }
        }
    ...
    img.src = heroImg(question.correctHero.name);
  }, [question, selected]);
          ...
          <div className="rounded-2xl bg-dota-card/80 border border-dota-border/50 p-6 flex flex-col items-center">
            {question.questionMode === 'silhouette' ? (
              <>
                ...
                <canvas ref={canvasRef} className="rounded-xl border border-purple-500/20 mb-2" style={{ width: 280, height: 160 }} />
                {selected && <img src={heroImg(question.correctHero.name)} alt="" className="w-[280px] h-[160px] rounded-xl object-cover mt-2 animate-fade-in" />}
              </>
            ) : (
              <>
