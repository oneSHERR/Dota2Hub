...
export function ProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="rounded-2xl bg-dota-card border border-dota-border p-6 mb-6">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-slate-800 text-slate-400 text-sm font-body hover:bg-slate-700 hover:text-white transition-all duration-300"
          >
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-4 p-1 rounded-2xl bg-dota-card border border-dota-border w-fit">
        {[
          ...
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-body font-medium transition-all duration-300 ${
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-dota-bg border border-dota-border text-white font-body text-sm placeholder:text-slate-600 focus:outline-none focus:border-dota-accent/50"
                />
              </div>
              <button onClick={handleSearch} className="px-4 py-2.5 rounded-2xl bg-dota-accent text-white font-body text-sm font-bold hover:bg-red-600 transition-all duration-300">
                Найти
              </button>
            </div>
            ...
            {searchResults.length > 0 && (
              <div className="mt-3 space-y-2">
                {searchResults.map((u: any) => (
                  <div key={u.uid} className="flex items-center justify-between p-3 rounded-2xl bg-dota-bg">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-blue-600/20 text-blue-400 text-xs font-body hover:bg-blue-600/30 transition-all duration-300"
                    >
            ...
            <div className="rounded-2xl bg-dota-card border border-dota-gold/30 p-5">
              ...
              <div className="space-y-2">
                {friendRequests.map((req: any) => (
                  <div key={req.uid} className="flex items-center justify-between p-3 rounded-2xl bg-dota-bg">
                    <span className="font-body text-sm text-white">{req.name}</span>
                    <button
                      onClick={() => handleAcceptRequest(req.uid)}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-green-600/20 text-green-400 text-xs font-body hover:bg-green-600/30 transition-all duration-300"
                    >
          ...
          <div className="rounded-2xl bg-dota-card border border-dota-border p-5">
            ...
            {friends.length > 0 ? (
              <div className="space-y-2">
                {friends.map((f: any) => (
                  <div key={f.uid} className="flex items-center gap-3 p-3 rounded-2xl bg-dota-bg">
          ...
          {matches.length > 0 ? (
            <div className="space-y-2">
              {matches.slice(0, 20).map((m: any) => {
                ...
                return (
                  <div key={m.id} className={`flex items-center gap-4 p-3 rounded-2xl bg-dota-bg border ${
...
function StatCard({ icon: Icon, value, label, color }: { icon: any; value: any; label: string; color?: string }) {
  return (
    <div className="rounded-2xl bg-dota-bg p-3 text-center">
