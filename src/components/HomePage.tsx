...
import { Swords, BookOpen, HelpCircle, ChevronRight, Zap, Shield, Target, ScrollText, TrendingUp, Trophy, Crown, Brain, Calculator } from 'lucide-react';
import { ALL_HEROES, META_HEROES } from '@/data/heroes';

const FEATURES = [
  { icon: Swords, title: 'Draft Arena', desc: '1v1 с друзьями или Solo Drafter против AI. Оценка S/A/B/C/D.', link: '/draft', color: 'from-red-500 to-orange-600', glow: 'shadow-red-500/20' },
  { icon: Target, title: 'Угадай контрпик', desc: 'Мини-квиз: угадай лучший контрпик из 4 вариантов. 3 уровня сложности.', link: '/counterpick', color: 'from-pink-500 to-rose-600', glow: 'shadow-pink-500/20' },
  { icon: Calculator, title: 'Калькулятор драфта', desc: 'Введи вражеский драфт — получи топ-15 лучших контрпиков по позициям.', link: '/calculator', color: 'from-cyan-500 to-blue-600', glow: 'shadow-cyan-500/20' },
  { icon: BookOpen, title: 'Wiki + Гайды', desc: 'Контрпики, синергии, предметы и советы для 15+ героев.', link: '/wiki', color: 'from-blue-500 to-cyan-500', glow: 'shadow-blue-500/20' },
  { icon: Trophy, title: 'Тир-лист', desc: 'S/A/B/C/D по позициям. Обновляется с OpenDota API.', link: '/tierlist', color: 'from-amber-500 to-yellow-600', glow: 'shadow-amber-500/20' },
  { icon: Crown, title: 'Лидерборд', desc: 'Рейтинг лучших драфтеров. Соревнуйся за первое место!', link: '/leaderboard', color: 'from-purple-500 to-indigo-600', glow: 'shadow-purple-500/20' },
];
...
const STATS = [
  ...
  { icon: Brain, value: 'Solo', label: 'Drafter' },
];
    ...
    <div className="relative overflow-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center px-4">
        ...
        <div className="relative z-10 text-center max-w-4xl">
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-dota-accent/10 border border-dota-accent/20 mb-8 backdrop-blur-sm">
            ...
            <span className="text-xs font-body text-dota-accent tracking-wider uppercase font-medium">Контрпик-квиз · Калькулятор драфта · OpenDota API — НОВОЕ</span>
          </div>
          ...
          <p className="text-lg sm:text-xl text-slate-400 font-body max-w-2xl mx-auto mb-10 leading-relaxed">
            Драфт-арена, гайды героев, тир-лист и квизы — всё для того, чтобы побеждать
          </p>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <div className="flex items-center gap-3 mb-8">
          ...
          <Link to="/tierlist" className="ml-auto flex items-center gap-1 text-sm font-body text-dota-gold hover:text-amber-300 transition-colors">
            Тир-лист <ChevronRight className="w-4 h-4" />
          </Link>
        </div>
