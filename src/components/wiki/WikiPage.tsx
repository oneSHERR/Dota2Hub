...
import { getHeroGuide, getHeroTier } from '@/data/heroGuides';
import { getAttrColor, getAttrLabel } from '@/lib/utils';
import type { Hero } from '@/types';
import { Search, X, Shield, Swords, Users, ChevronLeft, Zap, BookOpen, Star, Target, ArrowUp, ArrowDown, Award } from 'lucide-react';
    ...
    <div className="min-h-screen">
      <div className="relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 pt-10 pb-6">
          ...
          <p className="font-body text-slate-400 text-sm">{ALL_HEROES.length} героев · Патч 7.41b · Гайды и матчапы</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20"><p className="font-body text-slate-500 text-lg">Герой не найден</p></div>
        )}
      </div>
    </div>
  );
...
function HeroGrid({ heroes, onSelect, onHover, hoveredHero }: { heroes: Hero[]; onSelect: (h: Hero) => void; onHover: (h: Hero | null) => void; hoveredHero: Hero | null; }) {
  return (
    <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-1">
      {heroes.map(hero => {
        const tierData = getHeroTier(hero.name);
        return (
          <button key={hero.id} onClick={() => onSelect(hero)} onMouseEnter={() => onHover(hero)} onMouseLeave={() => onHover(null)}
            ...
            style={{ opacity: hoveredHero && hoveredHero.id !== hero.id ? 0.4 : 1, transition: 'opacity 0.2s, transform 0.2s' }}>
            ...
            <div className="absolute top-1 left-1 w-1.5 h-1.5 rounded-full opacity-80" style={{ backgroundColor: getAttrColor(hero.primary_attr) }} />
            {/* Tier badge */}
            {tierData && (
              <div className={`absolute top-0.5 right-0.5 text-[8px] font-display font-bold px-1 rounded ${
                tierData.tier === 'S' ? 'bg-amber-500/80 text-white' :
                tierData.tier === 'A' ? 'bg-emerald-500/80 text-white' :
                tierData.tier === 'B' ? 'bg-blue-500/80 text-white' :
                tierData.tier === 'C' ? 'bg-orange-500/80 text-white' :
                'bg-red-500/80 text-white'
              }`}>{tierData.tier}</div>
            )}
            <div className="absolute bottom-0 left-0 right-0 p-1.5">
              <span className="text-[9px] sm:text-[10px] font-body font-semibold text-white leading-tight block truncate text-center drop-shadow-lg">{hero.localized_name}</span>
            </div>
...
function HeroDetail({ hero, onBack }: { hero: Hero; onBack: () => void }) {
  const [tab, setTab] = useState<'guide' | 'counters' | 'synergy'>('guide');
  const guide = getHeroGuide(hero.localized_name);
  const tierData = getHeroTier(hero.name);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              {tierData && (
                <span className={`text-xs font-display font-bold px-2 py-0.5 rounded ${
                  tierData.tier === 'S' ? 'bg-amber-500/20 text-amber-400' :
                  tierData.tier === 'A' ? 'bg-emerald-500/20 text-emerald-400' :
                  tierData.tier === 'B' ? 'bg-blue-500/20 text-blue-400' :
                  tierData.tier === 'C' ? 'bg-orange-500/20 text-orange-400' :
                  'bg-red-500/20 text-red-400'
                }`}>Тир {tierData.tier}</span>
              )}
              {guide && (
                <span className="text-xs font-body px-2 py-0.5 rounded bg-white/10 text-slate-300">
                  {'⭐'.repeat(guide.difficulty)} Сложность
                </span>
              )}
            </div>
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-xl bg-[#111827] border border-white/5 w-fit">
        <button onClick={() => setTab('guide')} className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-body font-bold transition-all ${tab === 'guide' ? 'bg-dota-gold/15 text-dota-gold' : 'text-slate-500 hover:text-white'}`}>
          <BookOpen className="w-4 h-4" /> Гайд
        </button>
          ...
          {guide ? (
            <>
              ...
              <div className="rounded-2xl bg-[#111827] border border-white/5 p-5">
                <p className="text-sm font-body text-slate-300 leading-relaxed">{guide.description}</p>
                <p className="text-sm font-body text-slate-400 mt-3 italic">{guide.playstyle}</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-2xl bg-[#111827] border border-emerald-500/15 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-display text-base font-bold text-white">Сильные стороны</h3>
                  </div>
                  <div className="space-y-2">
                    {guide.strengths.map((s, i) => (
                      <div key={i} className="flex items-start gap-2 text-sm font-body text-slate-300">
                ...
                <div className="rounded-2xl bg-[#111827] border border-red-500/15 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-display text-base font-bold text-white">Слабые стороны</h3>
                  </div>
                  <div className="space-y-2">
              ...
              {/* Game phases */}
              <div className="rounded-2xl bg-[#111827] border border-white/5 p-5">
                <h3 className="font-display text-base font-bold text-white mb-4">Фазы игры</h3>
                <div className="space-y-3">
                  {[
                    { label: 'Ранняя игра', text: guide.earlyGame, color: 'text-emerald-400', bg: 'bg-emerald-500/8 border-emerald-500/15' },
                    { label: 'Мидгейм', text: guide.midGame, color: 'text-amber-400', bg: 'bg-amber-500/8 border-amber-500/15' },
                    { label: 'Лейтгейм', text: guide.lateGame, color: 'text-red-400', bg: 'bg-red-500/8 border-red-500/15' },
                  ].map(({ label, text, color, bg }) => (
                    <div key={label} className={`rounded-xl border p-4 ${bg}`}>
                      <span className={`text-xs font-body font-bold uppercase tracking-wider ${color}`}>{label}</span>
                      <p className="text-sm font-body text-slate-300 mt-2 leading-relaxed">{text}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Items */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-2xl bg-[#111827] border border-dota-gold/15 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Star className="w-4 h-4 text-dota-gold" />
                    <h3 className="font-display text-base font-bold text-white">Ключевые предметы</h3>
                  </div>
                  <div className="space-y-1.5">
                    {guide.coreItems.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-dota-gold/5">
                        <span className="text-xs font-mono text-dota-gold font-bold w-4">{i + 1}</span>
                ...
                <div className="rounded-2xl bg-[#111827] border border-blue-500/15 p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="font-display text-base font-bold text-white">Ситуативные</h3>
                  </div>
                  <div className="space-y-1.5">
                    {guide.situationalItems.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-blue-500/5">
              ...
              {/* Tips */}
              <div className="rounded-2xl bg-[#111827] border border-purple-500/15 p-5">
                <div className="flex items-center gap-2 mb-3">
                  <Award className="w-4 h-4 text-purple-400" />
                  <h3 className="font-display text-base font-bold text-white">Советы</h3>
                </div>
                <div className="space-y-2">
                  {guide.tips.map((tip, i) => (
                    <div key={i} className="flex items-start gap-2 p-2.5 rounded-lg bg-purple-500/5">
                      <span className="text-purple-400 text-sm mt-0.5 flex-shrink-0">💡</span>
                      <span className="text-sm font-body text-slate-300">{tip}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="rounded-2xl bg-[#111827] border border-white/5 p-8 text-center">
              ...
              <p className="text-sm font-body text-slate-400">Гайд для этого героя ещё не добавлен.</p>
              <p className="text-xs font-body text-slate-600 mt-1">Проверьте вкладки «Матчапы» и «Синергии» для информации о контрпиках.</p>
            </div>
          )}
        </div>
      )}
