  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        {/* Фильтры */}
        <div className="sticky top-0 z-20 bg-dota-bg/95 backdrop-blur-md border-b border-dota-border/20 -mx-4 px-4 py-3 mb-6">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="flex gap-1">
              {ATTR_TABS.map(({ key, label, color, attr }) => (
                <button
                  ...
                  className="relative flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-body font-bold uppercase tracking-wider transition-all duration-300"
                  style={{
                ...
                >
                  {attr && <AttrIcon attr={attr} size={16} />}
                  {label}
                </button>
              ))}
            </div>
            <div className="relative flex-1 max-w-xs ml-auto">
              ...
              <input
                ...
                className="w-full pl-10 pr-8 py-2.5 rounded-xl bg-white/[0.04] border border-dota-border/30 text-white font-body text-base placeholder:text-slate-600 focus:border-white/20 focus:outline-none transition-all duration-300"
              />
              ...
              grouped[key]?.length > 0 && (
                <div key={key}>
                  <div className="flex items-center gap-3 mb-4">
                    <AttrIcon attr={key === 'all' ? 'all' : key} size={20} />
    ...
    <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-1.5">
      {heroes.map(hero => (
        <button
          ...
          className="group relative aspect-[7/9] rounded-xl overflow-hidden transition-all duration-300 hover:z-10 hover:scale-110 hover:shadow-2xl hover:shadow-black/60"
          style={{
...
function HeroDetail({ hero, onBack }: { hero: Hero; onBack: () => void }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <button onClick={onBack} className="flex items-center gap-2 text-base font-body text-slate-400 hover:text-white mb-6 transition-all duration-300">
        <ChevronLeft className="w-5 h-5" /> Все герои
      </button>

      {/* ===== HERO HEADER — ФИКС: не растягивается ===== */}
      <div className="relative rounded-2xl overflow-hidden mb-8 bg-dota-card/60">
        {/* Фон — блюр картинка */}
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-end gap-6 p-6 sm:p-8">
          ...
          <div className="w-40 h-24 sm:w-56 sm:h-32 rounded-2xl overflow-hidden border-2 border-dota-border/30 shadow-2xl shadow-black/50 flex-shrink-0">
          ...
          <div className="text-center sm:text-left flex-1">
            <div className="flex items-center gap-3 mb-2 justify-center sm:justify-start">
              <AttrIcon attr={hero.primary_attr} size={20} />
            ...
            <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
              {hero.roles.map(role => (
                <span key={role} className="px-3 py-1.5 rounded-full bg-white/8 border border-dota-border/30 text-xs font-body text-slate-300 uppercase tracking-wider font-semibold">{role}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-8 p-1 rounded-2xl bg-dota-card/60 border border-dota-border/20 w-fit">
        <button onClick={() => setTab('abilities')} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-base font-body font-bold transition-all duration-300 ${tab === 'abilities' ? 'bg-amber-500/15 text-amber-400' : 'text-slate-500 hover:text-white'}`}>
          <BookOpen className="w-5 h-5" /> Способности
        </button>
        <button onClick={() => setTab('counters')} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-base font-body font-bold transition-all duration-300 ${tab === 'counters' ? 'bg-dota-accent/15 text-dota-accent' : 'text-slate-500 hover:text-white'}`}>
          <Swords className="w-5 h-5" /> Матчапы
        </button>
        <button onClick={() => setTab('synergy')} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-base font-body font-bold transition-all duration-300 ${tab === 'synergy' ? 'bg-green-500/15 text-green-400' : 'text-slate-500 hover:text-white'}`}>
      ...
      {tab === 'abilities' && (
        <div>
          ...
          {abilitiesError && (
            <div className="rounded-2xl bg-red-500/10 border border-red-500/20 p-6 text-center">
              ...
              <button
                ...
                className="px-5 py-2 rounded-xl bg-red-500/20 text-red-400 font-body font-bold hover:bg-red-500/30 transition-all duration-300"
              >
                Попробовать снова
              </button>
            </div>
          ...
          {!abilitiesLoading && !abilitiesError && heroAbilities.length === 0 && (
            <div className="rounded-2xl bg-dota-card/60 border border-dota-border/20 p-8 text-center">
      ...
      {tab === 'counters' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="rounded-2xl bg-dota-card/60 border border-green-500/15 p-6">
          ...
          <div className="rounded-2xl bg-dota-card/60 border border-red-500/15 p-6">
      ...
      {/* ===== SYNERGY TAB ===== */}
      {tab === 'synergy' && (
        <div className="rounded-2xl bg-dota-card/60 border border-blue-500/15 p-6">
          ...
          {synergies.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{synergies.slice(0, 12).map(({ hero: h, score }) => (
              <div key={h.id} className="flex items-center gap-3 p-3 rounded-2xl bg-white/[0.03] hover:bg-white/[0.04] transition-all duration-300">
                <img src={h.icon} alt="" className="w-9 h-9 rounded-xl" />
...
function AbilityCard({ ability, abilityKey, index }: { ability: any; abilityKey: string; index: number }) {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#111827] to-[#0d1117] border border-dota-border/20 overflow-hidden hover:border-dota-border/30 transition-all duration-300">
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center gap-4 p-5 text-left">
        {/* Иконка */}
        <div className="relative flex-shrink-0">
          {imgUrl ? (
            <img src={imgUrl} alt={ability.dname} className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border-2 border-dota-border/30 object-cover shadow-lg"
              onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          ) : (
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl border-2 border-dota-border/30 bg-white/[0.04] flex items-center justify-center">
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-dota-border/20 pt-4 space-y-4 animate-fade-in-up">
          ...
          <div className="flex flex-wrap gap-2">
            {behavior && <span className="px-3 py-1.5 rounded-full bg-white/[0.04] border border-dota-border/30 text-xs font-body text-slate-400 uppercase tracking-wider">{behavior}</span>}
          ...
          {ability.attrib && ability.attrib.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {ability.attrib.filter((a: any) => a.header && a.value).map((attr: any, j: number) => (
                <div key={j} className="flex items-center justify-between px-3 py-2 rounded-xl bg-white/[0.03]">
...
function MatchupRow({ hero, score, positive }: { hero: Hero; score: number; positive: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.04] transition-all duration-300">
      <img src={hero.icon} alt="" className="w-8 h-8 rounded-xl" />
      ...
      <div className="w-24 h-2 rounded-full bg-white/[0.04] overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-300 duration-500 ${positive ? 'bg-green-500' : 'bg-red-500'}`} style={{ width: `${barWidth}%` }} />
      </div>
